-- ====================================================================
-- Institutional Platform Database Schema Migration for Al-Rikabiyyah Platform
-- ====================================================================
...
-- --------------------------------------------------------------------
-- 1. PROFILES & ROLES & PERMISSIONS (AUTH & SECURITY)
-- --------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.roles (
  ...
  code TEXT UNIQUE NOT NULL, -- e.g. super_admin, finance_manager, genealogy_manager, member
...
CREATE TABLE IF NOT EXISTS public.permissions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  code TEXT UNIQUE NOT NULL, -- e.g. members:read, members:write, payments:approve
  name_ar TEXT NOT NULL,
...
-- Sequence for sequential membership numbers: RK-YYYY-XXXXXX
...
-- --------------------------------------------------------------------
-- 3. PAYMENT METHODS, RECEIPTS & TRANSACTIONS
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.payment_methods (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name_ar TEXT NOT NULL, -- بنكك, أوكاش, فوري, تحويل بنكي
...
CREATE TABLE IF NOT EXISTS public.payment_receipts (
  ...
  payment_type TEXT NOT NULL, -- 'membership', 'donation', 'social_support'
  transaction_reference TEXT UNIQUE NOT NULL,
  payment_date DATE NOT NULL DEFAULT CURRENT_DATE,
  receipt_path TEXT NOT NULL, -- Private Supabase storage path
  notes TEXT,
...
-- --------------------------------------------------------------------
-- 4. FINANCIAL ACCOUNTS, TRANSACTIONS, EXPENSES & BUDGETS
-- --------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.financial_transactions (
  ...
  category TEXT NOT NULL, -- 'membership_fee', 'donation', 'disbursement', etc.
...
CREATE TABLE IF NOT EXISTS public.financial_reports (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  title TEXT NOT NULL,
  report_period TEXT NOT NULL, -- e.g. "Q1 2026", "Annual 2025"
  summary TEXT,
  file_path TEXT,
...
-- --------------------------------------------------------------------
-- 5. DONATIONS, CAMPAIGNS & PROJECTS
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.donation_campaigns (
  ...
  description TEXT,
  image_url TEXT,
  category TEXT NOT NULL, -- 'takaful', 'education', 'health', 'orphans', 'general'
  target_amount NUMERIC(12, 2) NOT NULL DEFAULT 0.00,
  current_amount NUMERIC(12, 2) NOT NULL DEFAULT 0.00, -- Only approved payments update this
  start_date DATE,
  end_date DATE,
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- --------------------------------------------------------------------
-- 6. SOCIAL SUPPORT & TAKAFUL SYSTEM
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.social_cases (
  ...
  category TEXT NOT NULL, -- 'health', 'education', 'emergency', 'orphan', 'marriage'
...
-- --------------------------------------------------------------------
-- 7. GENEALOGY & LINEAGE SYSTEM (FAMILIES, BRANCHES, NODES)
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 8. BUSINESS NETWORK, SPONSORS & JOBS
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 9. GOVERNANCE (ORGANIZATIONS, LEADERSHIP & DEPARTMENTS)
-- --------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.leadership_members (
  ...
  full_name TEXT NOT NULL,
  role_title TEXT NOT NULL, -- e.g. "رئيس المجلس", "الأمين العام"
  department_id UUID REFERENCES public.departments(id) ON DELETE SET NULL,
  state TEXT,
  bio TEXT,
  photo_url TEXT,
  display_order INTEGER DEFAULT 0,
  is_leadership_body BOOLEAN DEFAULT FALSE, -- 26-member Leadership Council
...
-- --------------------------------------------------------------------
-- 10. CONTENT MANAGEMENT & OFFICIAL DIGITAL ARCHIVE
-- --------------------------------------------------------------------
...
-- Sequence for sequential document numbers: RK-DOC-YYYY-XXXX
...
CREATE TABLE IF NOT EXISTS public.documents (
  ...
  title TEXT NOT NULL,
  category TEXT NOT NULL, -- 'bylaw', 'decision', 'minutes', 'historical', 'report'
  description TEXT,
  file_path TEXT NOT NULL,
  file_size INTEGER,
...
-- --------------------------------------------------------------------
-- TRIGGERS & FUNCTIONS FOR SEQUENTIAL NUMBERING
-- --------------------------------------------------------------------
...
-- Public read policies for open catalog tables
...
-- Profile RLS: Users can read and edit their own profiles
CREATE POLICY "Users read own profile" ON public.profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users update own profile" ON public.profiles FOR UPDATE USING (auth.uid() = id);

-- Member RLS: Member can view their own membership record
CREATE POLICY "Users read own member record" ON public.members FOR SELECT USING (auth.uid() = user_id);

-- Payment Receipts RLS: Users can view and upload their receipts
...
-- Service Role Bypass for Backend API / Server Actions
